...
#os.system("touch k-d-prec-mprec.csv")
# Get recommendations from predict.py and time the thing
kd=open("k-d-prec-mprec.csv","a")
kd.write("K, d, Prediction Time, Prec, mPrec\n")
kd.close()
print(y.shape)
for k in range(50,51):
	for p in range(4,5):
		kd=open("k-50-d-5-prec-mprec.csv","a")
		tic = tm.perf_counter()
		yPred = predict.getReco( X, 5 , k, p)
		toc = tm.perf_counter()
		##edited
		print(yPred)
		##edited	
		...
		##edited
		predtime=predtime+",%0.3f" % preck[0]
		predtime=predtime+",%0.3f" % preck[2]
		predtime=predtime+",%0.3f" % preck[4]
		print("Prec Written")
		##edited
		...
		##edited
		predtime=predtime+",%0.3f" % mpreck[0]
		predtime=predtime+",%0.3f" % mpreck[2]
		predtime=predtime+",%0.3f" % mpreck[4]+"\n"
		kd.write(predtime)
		print("mPrec Written")
		kd.close()
