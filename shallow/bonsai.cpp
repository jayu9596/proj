...
vector<_int> kmeansplusplus(SMatF *mat, _int K){
  ...
  set_d_with_s( mat->data[start], mat->size[start], centers[0] );
  ...
  for(_int l = 0; l < K; l++){
  	  //cout <<"2"<<endl;
  		for(_int i = 0; i < nc; i++ ){
  			  //cout <<"3"<<endl;
  			for(_int j = 0; j<  v.size() ; j++){
  				if(mask[i])continue;
  				cosines[j][i] = mult_d_s_vec( centers[j], mat->data[i], mat->size[i] );
  				if (cosines[j][i] >= max_dist) max_dist_index = i;
  			}
  		}
  		v.push_back(max_dist_index);
  		mask[max_dist_index] =1;
  	}
